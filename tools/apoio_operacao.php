...
<html>
	<head>
		<title>Ferramentas de Administrador - Conte&uacute;dos</title>
...
<table cellpadding="10" cellspacing="0" border="0" width="90%"  align="left">
	<tr>
		<td align=left width="60%">		
		<?php
		if (isset($_REQUEST["APOIO_ENVIO"]))
		{
			if ( $_REQUEST["APOIO_ENVIO"] == "alterar" )
				echo " <br><b>Arquivo Alterado com Sucesso</b>";		
			else
				if ( $_REQUEST["APOIO_ENVIO"] == "inserir" )
					echo " <br><b>Arquivo Inserido com Sucesso</b>";
		 }		
		else
		{
			echo "<br><b>Conte&uacute;dos - " . $_REQUEST["OPCAO"] . "</b>";
			
			if ( $_REQUEST["OPCAO"] == "Alterar" )
			{
				echo "<br><br>" . "\n";
				echo "<table><tr><td>" . "\n";
				echo "<a href=\"#\" onClick=\"if (confirm('Deseja mesmo excluir este arquivo ?')) { window.open('apoio_envio.php?PAGINA2=".$_REQUEST['PAGINA']."&OPCAO=Remover&PAGINA=apoio&COD_ARQUIVO=" . $_REQUEST["COD_ARQUIVO"] ."','Wlocal','top=106,left=160,width=300px,height=200px,toolbar=no,status=yes,menubar=no,scrollbars=auto,scrolling=auto,resizable=yes') }\">" . "\n";
		...
		<? if($_REQUEST['PAGINA']=='instancia'){
			echo "<a href=\"".$url."/apoio/index.php\">Voltar para Conteúdos</a>";	
		}else{
	...
	if ( ($_REQUEST["OPCAO"] != "Inserir") AND ( $_REQUEST["COD_ARQUIVO"] != "" ) )
	{
		...
		$cam_local = str_replace("\\", "/",$linhaN["CAMINHO_LOCAL_ARQUIVO"]);
	...
	<form name="form1" method="post" enctype="multipart/form-data" action="apoio_envio.php?OPCAO=<?=$_REQUEST["OPCAO"]?>">
		<tr>
			<td align=left width="60%">
			...
			?>
				<td width="40%" valign="top">
					...
					<iframe name="locais" src="apoio_local.php?PAGINA=<?=$_REQUEST['PAGINA']?>&COD_ARQUIVO=<?=$arquivo?>" 
          frameborder=0 style="position:absolute; width:300px; height:500px; z-i0ndex: 3; 
		...
		<tr>
			<td align="center">
				...
				<?if($_REQUEST['PAGINA']=='instancia'){?>
						<input type="reset" value="Cancelar" onClick="window.location.href = '<?=$url?>/apoio/index.php';" name="reset">
						<?}else{?>
						<input type="reset" value="Voltar para Conte&uacute;dos sem salvar" onClick="window.location.href = 'apoio.php';" name="reset"><?}?>
