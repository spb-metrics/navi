...
<table cellpadding="10" cellspacing="0" border="0" width="85%"  align="center">
    ...
    <td colspan="5" align="center"><font size="4"><b>Avaliação</b></font></td>
	</tr>
	...
	<tr>	
		...
		?>	
			<td colspan="3" align="left">
				<a href="avaliacao_operacao.php?PAGINA=<?=$_REQUEST['PAGINA'];?>&OPCAO=Inserir">Inserir novo Arquivo</a>				
			</td>
...
<? if($_REQUEST['PAGINA']=='instancia'){
		...
		?>
			<td colspan="1" align="left">
				<a href="avaliacao_operacao.php?OPCAO=Inserir">Inserir novo Arquivo</a>				
			</td>
...
<? if($_REQUEST['PAGINA']=='instancia'){
	...
	<tr>
		...
		if ($acesso == 1)
		{
			...
			<td align="center"><a href="?FILTRO=suas">Suas Avalições</a></td>	
		...
		if ($acesso == 2 or $acesso == 3)
		{
			...
			?>
			<td align="center"><a href="?FILTRO=suas">Suas Avaliações</a></td>	
  		<td align="center"><a href="?FILTRO=instanciaAtual">Publicados <?=$nivel->nome?> atual</td>
			<?php
		 }
...
{		
		...
		$rsConN = listaAvaliacaoAdm($_SESSION["codInstanciaGlobal"], $filtro, $quem);		
		...
		if ($rsConN)
		{
			if ($linhaN = mysql_fetch_array($rsConN))
			{
				...
				<tr> <td colspan="3"><p><font color=red>AVISO: Ao retirar o texto neste local, o arquivo não mais existirá .</font></p>  </td> </tr>
				<?php
				...
				while ($linhaN) 
				{
					echo "<tr>\n".
						 "	<td align=\"center\">\n".
						 "		<a href=\"#\" onClick=\"if (confirm('Deseja mesmo excluir este arquivo ?')) { window.open('avaliacao_envio.php?PAGINA=".$_REQUEST['PAGINA']."&OPCAO=Remover&COD_ARQUIVO=" . $linhaN["COD_ARQUIVO"] . "','Wlocal','top=106,left=160,width=300px,height=200px,toolbar=no,status=yes,menubar=no,scrollbars=yes,scrolling=yes,resizable=yes') }\">\n".
						 "			<img src=\"../imagens/remove.gif\" border=0 alt=\"Remover\">\n".
						 "		</a>\n&nbsp;&nbsp;&nbsp;\n".
						 "		<a href=\"avaliacao_operacao.php?PAGINA=".$_REQUEST['PAGINA']."&OPCAO=Alterar&COD_ARQUIVO=".$linhaN["COD_ARQUIVO"]."\">\n".
