...
<table cellpadding="10" cellspacing="0" border="0" width="90%"  align="center">
	<tr>
		...
		<?php
		if (isset($_REQUEST["EXERCICIO_ENVIO"]))
		{
			if ( $_REQUEST["EXERCICIO_ENVIO"] == "alterar" )
				echo " <br><b>Arquivo Alterado com Sucesso</b>";		
			else
				if ( $_REQUEST["EXERCICIO_ENVIO"] == "inserir" )
					echo " <br><b>Arquivo Inserido com Sucesso</b>";
		 }		
		else
		{
			echo "<br><b> Exercicios - " . $_REQUEST["OPCAO"] . "</b>";
			
			if ( $_REQUEST["OPCAO"] == "Alterar" )
			{
				echo "<br><br>" . "\n";
				echo "<table><tr><td>" . "\n";
				echo "<a href=\"#\" onClick=\"if (confirm('Deseja mesmo excluir este arquivo ?')) { window.open('exercicio_envio.php?PAGINA2=".$_REQUEST['PAGINA']."&OPCAO=Remover&PAGINA=exercicio&COD_ARQUIVO=" . $_REQUEST["COD_ARQUIVO"] ."','Wlocal','top=106,left=160,width=300px,height=200px,toolbar=no,status=yes,menubar=no,scrollbars=yes,scrolling=yes,resizable=yes') }\">" . "\n";
		...
		<? if($_REQUEST['PAGINA']=='instancia'){
			echo "<a href=\"".$url."/exercicio/index.php\">Voltar para exercício</a>";	
		}else{
	...
	if ( ($_REQUEST["OPCAO"] != "Inserir") AND ( $_REQUEST["COD_ARQUIVO"] != "" ) )
	{
		...
		$cod_arquivo= $_REQUEST["COD_ARQUIVO"];
			 
	
		$rsConN = listaExer($_REQUEST["COD_ARQUIVO"],"","");
	...
	<form name="form1" method="post" enctype="multipart/form-data" action="exercicio_envio.php?OPCAO=<?=$_REQUEST["OPCAO"]?>">
		<tr>
			<td align="left" width="60%">
			...
			?>
				<td width="40%" valign="top">
					...
					<iframe name="locais" src="exercicio_local.php?PAGINA=<?=$_REQUEST['PAGINA'];?>&COD_ARQUIVO=<?=$arquivo?>" frameborder=0 style="position:absolute; width:300px; height:500px; z-index: 3; overflow: visible; visibility: visible">
					</iframe>
				</td>
			<?php
			 }
		...
		<tr>
			<td align="center">
				...
				<?if($_REQUEST['PAGINA']=='instancia'){?>
						<input type="reset" value="Cancelar" onClick="window.location.href = '<?=$url?>/exercicio/index.php';" name="reset">
						<?}else{?>
						<input type="reset" value="Cancelar" onClick="window.location.href = 'exercicio.php';" name="reset"><?}?>
