...
//nivel e instancia atuais da navegacao
...
switch($_REQUEST["OPCAO"]) {
    ...
    //old-style, passando pk e chave ao inves do objeto
    //$instancias = $nivelGerenciado->getInstancias($codInstanciaNivelPai,$pkPai);     
    //$instancias = $nivelGerenciado->getInstancias($paiFiltrar);
    
    //usa getRegistros para editar somente a tabela de instancias (1:N) ou nomes (N:M)
    $instancias = $nivelGerenciado->getRegistros($paiFiltrar);
    ...
    if  (!empty($instancias->records)) {		    
      ...
      $chave = $nivelGerenciado->nomeFisicoPK;
      $nome  = $nivelGerenciado->nomeFisicoCampoNome;
      ...
      foreach($instancias->records as $instancia) {  
        ...
        if ($codInstanciaAtual!=$instancia->$chave || $nivelAtual->codNivel!=$nivelGerenciado->codNivel) {
          ...
          echo "<a href=\"".$_SERVER["PHP_SELF"]."?OPCAO=Excluir&frm_codInstanciaNivel=".$instancia->$chave.'&frm_nome='.$instancia->$nome."&voltar=".$_SERVER["PHP_SELF"]."\"><img src=\"../imagens/remove.gif\" border=0 alt=\"Remover\" title=\"Remover\"></a>&nbsp;&nbsp;&nbsp;";          
        }
        else {
          echo 'Atual';
        }
        ...
        echo "<a href=\"".$_SERVER["PHP_SELF"]."?OPCAO=Alterar&frm_codInstanciaNivel=".$instancia->$chave.'&frm_nome='.$instancia->$nome."\"><img src=\"../imagens/edita.gif\" border=0 alt=\"Remover\" title=\"Alterar\"></a>";
        echo "</td>";
    ...
    echo '<div style="float:left;padding-left:150px;">';
    //Alem da chave unica, Repassa a chave fraca tambem, 
    //pois, para instancias NxM, o que interessa no CRUD é a chaveFraca, para o "update tabela where pk=.."
    ...
    if ($nivelGerenciado->tipoRelacionamentoComNivelPai==1) {
      ...
      echo '<select name="frm_codInstanciaPai">';
    ...
    echo '<div style="text-align:left">Nome/Descri&ccedil;&atilde;o de '.$nivelGerenciado->nome.'<br>';
    echo '<input type="text" size="60" maxlength="80" name="frm_nomeInstancia" value="'.$_REQUEST['frm_nome'].'"></div>';
		if (!empty($nivelGerenciado->nomeFisicoCampoAbreviatura)) {
    ...
    if ($nivelGerenciado->tipoRelacionamentoComNivelPai==2) {
      ...
      mostraRelacionamentosInstancia($nivelGerenciado,(int)$_REQUEST['frm_codInstanciaNivel']);  //nesse caso frm_codInstanciaNivel é a chave fraca
      echo '</div>';
    }
    ...
    $chaveInstancia = $_REQUEST["frm_codInstanciaNivel"]; 
    
    $sucessoRelacionamentos = $nivelGerenciado->salvarRelacionamentos($_REQUEST['instanciasPai'],$chaveInstancia);    
		$sucessoInstancia = $nivelGerenciado->alteraInstancia($chaveInstancia ,$_REQUEST["frm_nomeInstancia"],$_REQUEST["frm_abreviatura"],$_REQUEST["frm_codInstanciaPai"]);

		if ($sucessoRelacionamentos && $sucessoInstancia) {
    ...
    $chaveInstancia = $_REQUEST["frm_codInstanciaNivel"]; 
		...
		echo "<p>Voc&ecirc; realmente deseja excluir a(o) ".$nivelGerenciado->nome." ".$_REQUEST['frm_nome']."? Essa a&ccedil;&atilde;o n&atilde;o poder&aacute; ser desfeita.</p>";
		echo "<br>";
		echo "<input type=\"button\" value=\"Excluir\" onclick=\"window.location.href = '".$_SERVER["PHP_SELF"]."?OPCAO=ExcluirMake&frm_codInstanciaNivel=".$chaveInstancia."'\">";
		echo "<input type=\"button\" value=\"Cancelar\" name=\"cancel\" onclick=\"window.location.href ='".$_REQUEST['voltar']."';\">";		
    ...
    // A EXCECAO É COMUNIDADES, FEITA NO SCRIPT /alunos/acoes_comunidade.php, 
    // separado para o modulo de comunidade controlar diretamente a saida de integrantes ou exclusao da comunidade sem incluir este arquivo
...
function mostraRelacionamentosInstancia($nivel,$chaveFraca) {
  $nivelPai = $nivel->getNivelAnterior();
  $pais = $nivel->buscaTodosSuperiores($chaveFraca);
    
  if (!empty($pais->records)) {   
    echo 'Rela&ccedil;&otilde;es com as inst&acirc;ncias superiores,  do n&iacute;vel <b>'.$nivelPai->nome.'</b><br>(cada relacionamento &eacute; uma inst&acirc;ncia do NAVi)<br /><br />';
    foreach($pais->records as $p) {  
      if ($p->chaveFraca==$chaveFraca) { $checked=' checked '; } else { $checked=' '; }    
      echo '<input type="checkbox" '.$checked.' value="'.$p->chaveFracaPai.'" name="instanciasPai[]">'.$p->nomePai.'<br>';
    }    
  }  
}
?>
