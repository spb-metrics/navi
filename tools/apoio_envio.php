...
<?php
  ...
  switch ( $_REQUEST["OPCAO"])	{
    ...
    /*  if(substr($_REQUEST["CAM_LOCAL"],0,4)=='http' )		{
				$sucesso = ApoioAltera($_REQUEST["COD_ARQUIVO"], $desc_arquivo, $_REQUEST["CAM_LOCAL"], "", "");
				if($sucesso) {							
          echo "<script> location.href=\"./apoio_operacao.php?PAGINA=".$_REQUEST['PAGINA']."&OPCAO=Alterar&COD_ARQUIVO=". $_REQUEST["COD_ARQUIVO"] ."&APOIO_ENVIO=alterar\"</script>";
				}
				else {
    	...
    	if ( (! $erro) and ($_FILES["ARQUIVO_NOVO"]["size"] == 0) )		{	
            ...
            if(strlen($_REQUEST["LINK_NOVO"])>7){//teste para saber se alguem colocou um link ao inves de arquivo
                 $sucesso =  ApoioAltera($_REQUEST["COD_ARQUIVO"], $desc_arquivo,$_REQUEST["LINK_NOVO"],"","");
  				  }
            else{
      ...
      // Cria os diretorios Locais - 	d:\upload\cursosnavi\\texto
  	  ...
  	  if ( (! $erro) and ($_FILES["ARQUIVO_NOVO"]["size"] > 0) ) {
  		  ...
  		  $CAMINHO = $caminhoUpload."/texto/". confNum($_SESSION["COD_PESSOA"]) . "/";
        ...
        $temp = explode('/',$cam_local); $nomeAntigo = $temp[count($temp)-1];
            ...
            duplica($CAMINHO . $_FILES["ARQUIVO_NOVO"]["name"],$_FILES["ARQUIVO_NOVO"]["name"],'texto/'. confNum($_SESSION["COD_PESSOA"])) ;
            //echo "<br><br> O arquivo foi carregado com sucesso. ";
                ...
                delete_via_ftp($cam_local);
      			...
      			$CAMINHO ="/texto/". confNum($_SESSION["COD_PESSOA"]) . "/";
      ...
      if ( (!$erro) and ($sucesso) ) {
		...
		case ("Inserir"):
		  ...
		  if($_REQUEST["LINK_NOVO"] AND ($_REQUEST["LINK_NOVO"]!= "http://") )	{
			  ...
			  $sucessolink = ApoioInsere($desc_arquivo,  $_REQUEST["LINK_NOVO"], "0", "");
 
			  if($sucessolink)	 {
				 
			    $num_arquivo =ApoioCodigo($desc_arquivo,$_REQUEST["LINK_NOVO"], "", "");
				  echo "<script> location.href=\"./apoio_operacao.php?PAGINA=".$_REQUEST['PAGINA']."&OPCAO=Alterar&COD_ARQUIVO=". $num_arquivo ."&APOIO_ENVIO=inserir\";</script>";
			  }
			  else {
