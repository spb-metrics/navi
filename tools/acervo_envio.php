...
<?php
  ...
  switch ( $_REQUEST["OPCAO"])	{
    ...
    /*  if(substr($_REQUEST["CAM_LOCAL"],0,4)=='http' )		{
				$sucesso = AcervoAltera($_REQUEST["COD_ARQUIVO"], $desc_arquivo, $_REQUEST["CAM_LOCAL"], "", "");
				if($sucesso) {							
          echo "<script> location.href=\"./acervo_operacao.php?PAGINA=".$_REQUEST['PAGINA']."&OPCAO=Alterar&COD_ARQUIVO=". $_REQUEST["COD_ARQUIVO"] ."&ACERVO_ENVIO=alterar\"</script>";
				}
				else {
        ...
        //	echo "<br><br> Sem alteração de arquivo.";
              ...
              echo "Os seguintes caracteres não sao permitidos em nomes de arquivos: \\ / : * ? < > | \"";
  				...
  				if (! $erro)	{
  					...
  					$rsConN = listaAcervo($_REQUEST["COD_ARQUIVO"],"","");
  					$linhaN = mysql_fetch_array($rsConN);								
  
  					//$cam_local_novo_2 = str_replace("////", "/", $cam_local_novo);										
  					//$sucesso = AcervoAltera($_REQUEST["COD_ARQUIVO"], $desc_arquivo, $cam_local_novo_2, $linhaN["TAMANHO_ARQUIVO"], $linhaN["TIPO_ARQUIVO"]);													
  					//$sucesso = AcervoAltera($_REQUEST["COD_ARQUIVO"], $desc_arquivo, $cam_local_novo, $linhaN["TAMANHO_ARQUIVO"], $linhaN["TIPO_ARQUIVO"]);
            $sucesso =  AcervoAltera($_REQUEST["COD_ARQUIVO"], $desc_arquivo, $cam_local_novo, $linhaN["TAMANHO_ARQUIVO"], $linhaN["TIPO_ARQUIVO"]);
    		   ...
    		   // echo "<br><br> Sem alteração no nome";
    			 ...
    			 //$sucesso = AcervoAltera($_REQUEST["COD_ARQUIVO"], $desc_arquivo, $cam_local_2, $linhaN["TAMANHO_ARQUIVO"], $linhaN["TIPO_ARQUIVO"]);													
    			 //$sucesso = AcervoAltera($_REQUEST["COD_ARQUIVO"], $desc_arquivo, $cam_local, $linhaN["TAMANHO_ARQUIVO"], $linhaN["TIPO_ARQUIVO"]);
           if(strlen($_REQUEST["LINK_NOVO"])>7){//teste para saber se alguem postou um link
  				     $sucesso =  AcervoAltera($_REQUEST["COD_ARQUIVO"], $desc_arquivo, $_REQUEST["LINK_NOVO"],"","");
  				  }
            else{
      ...
      // Cria os diretorios Locais - 	d:\upload\cursosnavi\\acervo
  	
  	  if ( (! $erro) and ($_FILES["ARQUIVO_NOVO"]["size"] > 0) ) {
  	
  		  $CAMINHO = $caminhoUpload."/acervo/". confNum($_SESSION["COD_PESSOA"]) . "/";
        ...
        $temp = explode('/',$cam_local); $nomeAntigo = $temp[count($temp)-1];
            ...
            duplica($CAMINHO . $_FILES["ARQUIVO_NOVO"]["name"],$_FILES["ARQUIVO_NOVO"]["name"],'acervo/'. confNum($_SESSION["COD_PESSOA"])) ;
            //echo "<br><br> O arquivo foi carregado com sucesso. ";
                ...
                delete_via_ftp($cam_local);
      			...
      			$CAMINHO ="/acervo/". confNum($_SESSION["COD_PESSOA"]) . "/";
      			//$CAMINHO_2 = str_replace("////", "/", $CAMINHO);							
      			//$sucesso = AcervoAltera($_REQUEST["COD_ARQUIVO"], $desc_arquivo, $CAMINHO_2 . $_FILES["ARQUIVO_NOVO"]["name"], $_FILES["ARQUIVO_NOVO"]["size"], $_FILES["ARQUIVO_NOVO"]["type"] );
      			$sucesso = AcervoAltera($_REQUEST["COD_ARQUIVO"], $desc_arquivo, $CAMINHO . $_FILES["ARQUIVO_NOVO"]["name"], $_FILES["ARQUIVO_NOVO"]["size"], $_FILES["ARQUIVO_NOVO"]["type"] );
      ...
      if ( (!$erro) and ($sucesso) ) {
		...
		case ("Inserir"):
		  ...
		  if($_REQUEST["LINK_NOVO"] AND ($_REQUEST["LINK_NOVO"]!= "http://") )	{
			  ...
			  $sucessolink = AcervoInsere($desc_arquivo,  $_REQUEST["LINK_NOVO"], "0", "");
 
			  if($sucessolink)	 {
				 
			    $num_arquivo =AcervoCodigo($desc_arquivo,$_REQUEST["LINK_NOVO"], "", "");
				  echo "<script> location.href=\"./acervo_operacao.php?PAGINA=".$_REQUEST['PAGINA']."&OPCAO=Alterar&COD_ARQUIVO=". $num_arquivo ."&ACERVO_ENVIO=inserir\";</script>";
			  }
			  else {
	...
	// Cria os diretorios Locais - 	d:\upload\cursosnavi\\acervo\\
    	...
    	if (! $erro)	{
    	  $CAMINHO = "/acervo/". confNum($_SESSION["COD_PESSOA"]) . "/";
    		//$CAMINHO_2 = str_replace("////", "//", $CAMINHO);
    		//$sucesso = AcervoInsere($desc_arquivo, $CAMINHO_2 . $_FILES["ARQUIVO_NOVO"]["name"], $_FILES["ARQUIVO_NOVO"]["size"], $_FILES["ARQUIVO_NOVO"]["type"]);
    		$sucesso = AcervoInsere($desc_arquivo, $CAMINHO . $_FILES["ARQUIVO_NOVO"]["name"], $_FILES["ARQUIVO_NOVO"]["size"], $_FILES["ARQUIVO_NOVO"]["type"]);
    	}				
    	...
    	if ( (! $erro) and ($sucesso) )	{
    	  $CAMINHO = "/acervo/". confNum($_SESSION["COD_PESSOA"]) . "/";
    		...
    		//$num_arquivo = AcervoCodigo($desc_arquivo, $CAMINHO_2 . $_FILES["ARQUIVO_NOVO"]["name"], $_FILES["ARQUIVO_NOVO"]["size"], $_FILES["ARQUIVO_NOVO"]["type"]);
    		$num_arquivo = AcervoCodigo($desc_arquivo, $CAMINHO . $_FILES["ARQUIVO_NOVO"]["name"], $_FILES["ARQUIVO_NOVO"]["size"], $_FILES["ARQUIVO_NOVO"]["type"]);
		...
		case ("Remover"):
			//if ( AcervoVerificaAcesso($_REQUEST["COD_ARQUIVO"]) )	{
				...
				if ( (! $rsConN) or (mysql_num_rows($rsConN) == 0) )	{
					echo "<br><br> Arquivo não encontrado.<BR><BR>";
					$erro = true;
				}
        //somente o dono do arquivo pode deletar, se for prof
				if (!$erro && !podeInteragir($_SESSION['userRole'],$_SESSION['interage']))	{
          echo "<br><br> Sem permissao.<BR><BR>";
					$erro = true;
				}
        else {
  				//busca linha do banco com os dados do arquivo
  				$linhaN = mysql_fetch_array($rsConN);
  			}
				
				if (!$erro && $linhaN['COD_PESSOA']!=$_SESSION['COD_PESSOA'] && Pessoa::isAdm($_SESSION['userRole'])) {
          echo "<br><br> Apenas o dono do arquivo pode exclui-lo.<BR><BR>";
            ...
            delete_via_ftp($linhaN["CAMINHO_LOCAL_ARQUIVO"]);
  				...
  				//apaga o registro
    			$sucesso = AcervoExclue($_REQUEST["COD_ARQUIVO"]);  
			  }
  			...
  			if ( (! $erro) and $sucesso )		{
  				...
  				if ( $_REQUEST["PAGINA"] == "acervo" )
  					echo "<script> window.opener.location.href='./acervo.php?PAGINA=".$_REQUEST['PAGINA2']."'; </script>";
  				else
