		...
		{ 
			if ( $_REQUEST["SENT"] == "" )
			{
				...
				echo "<br>".
					 ...
					 "<tr><td><b>Descrição do texto para ".$nivel->nome." : </b>&nbsp;&nbsp;</td></tr><tr><td> <input type=\"text\" name=\"DESC_ARQUIVO_INSTANCIA\" size=\"50\"value='".getDescricaoArquivo($_REQUEST["COD_ARQUIVO"])."' ></td></tr>\n".
					...
					// "<option value=3>Publico e Restrito</option>\n".
					 ...
					 "<tr><td colspan=2 align=left ><br><br><input type=\"button\" value=\"Enviar\"  onclick=\"javascript:if (document.form.DESC_ARQUIVO_INSTANCIA.value=='') { alert('Preencha a descrição do texto!'); document.form.DESC_ARQUIVO_INSTANCIA.focus(); } else { document.form.submit(); }  \">&nbsp;<input type=\"reset\" value=\"Cancelar\" onclick=\"window.location.href = 'avaliacao_local.php?COD_ARQUIVO=".$_REQUEST["COD_ARQUIVO"]."';\"></td></tr>\n".
					 "</table></form>\n";
			 }
			else
			{
			  ...
			  $nivel = getNivelAtual();
				...
				<form name="form" method="post" action="<?=$acao?>">				
					<table align="center">
						...
						<tr><td><b>Descrição do texto para <?=$nivel->nome?> :</b>&nbsp;&nbsp;</td></tr><tr><td> <?=$linhaL["DESC_AVALIACAO_INSTANCIA"]?> </td></tr>
	...
	{ 
		if ( AvaliacaoVerificaAcesso($_REQUEST["COD_ARQUIVO"]) )
		{
			...
			{ 
				...
				<form name="form" method="post" action="<?=$acao?>">				
					<table align="center">
						<tr><td><b>Descrição do texto para <?=$nivel->nome?> :</b>&nbsp;&nbsp;</td></tr><tr><td> <input type="text" name="DESC_ARQUIVO_INSTANCIA" size="50" value="<?=$linhaL["DESC_AVALIACAO_INSTANCIA"]?>" ></td></tr>
						<tr><td><b>Tipo de Acesso :</b>&nbsp;&nbsp;&nbsp;
						    ...
						    <select name="TIPO_ACESSO_NOVO">
								...
								<option value=3<? if ($linhaL["COD_TIPO_ACESSO"] == 3 ) echo " selected";?>>Público</option>
						    </select>
						    </td>
						</tr>
	...
	default:
		...
		<table width="100%" border=0 cellpadding=0 cellspacing=0>
			...
			{	
				...
				$rsConL = listaAvaliacaoLocal($_REQUEST["COD_ARQUIVO"]); // $rsConL é setado novamente
				echo mysql_error();
