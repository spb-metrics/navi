    ...
    if ($ok) {
      ...
      if ($sucesso) {
        ...
        if ($sucesso) {
          ...
          echo '<div><br><a href="'.$url.'./inscricao">Voltar</a></center></div>';                
        }      
      }
    }
    else {
    ...
    if (empty($_SESSION['COD_PESSOA'])) {
      ...
      if ($sucesso) {
        ...
        if (empty($_SESSION['COD_AL'])) {
            ...
            // RAZOAVELMENTE SERIA BOM INSERIR UM CÓIGO DE ALUNO NOVO
            $strSQL = "INSERT INTO aluno COD_PESSOA = '".$_SESSION['COD_PESSOA'].
            "';SELECT COD_AL FROM aluno WHERE COD_PESSOA = '".
            $_SESSION['COD_PESSOA']."'";
