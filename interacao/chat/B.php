...
$par=0; //exibicao das linhas

//gera o conteudo do javascript que acrescentará localmente as mensagens na pagina visualizada
function talk_print_message($codPessoa,$sender_id, $nomeParticipante, $receiver_id, $message_text, $timestamp, $reservado, $cor) {
  global $sUserID,$sRoomID,$output,$par;
  ...
  //trata nome do participante e sender id para nao ter bug nos nomes
  $nomeParticipante = str_replace( array ( '&', '"', "'", '<', '>', "\r\n" , "\n" ), array ( '&amp;' , '&quot;', '&#39;' , '&lt;' , '&gt;', '<br>', '<br>' ), $nomeParticipante); 
  $sender_id= str_replace( array ( '&', '"', "'", '<', '>', "\r\n" , "\n" ), array ( '&amp;' , '', '&#39;' , '&lt;' , '&gt;', '<br>', '<br>' ), $sender_id); 
  //começa a montar a string de retorno 
  $strOutput .= "<span title=\"".$sender_id.", ".date("H:i:s",$timestamp)."\" style=\"font-weight:bold;\">".$nomeParticipante."</span>";
  //$strOutput .=$sender_id;
  if ($reservado == 1) {
	    $strOutput .= str_replace( array ( '&', '"', "'", '<', '>', "\r\n" , "\n" ), array ( '&amp;' , '&quot;', '&#39;' , '&lt;' , '&gt;', '<br>', '<br>' ), STR_MSG_RESERVED . $receiver_id );
	}
  else 	{
    $strOutput .= STR_MSG_TALK;
		if ($receiver_id != "#".$sRoomID) {
			 $strOutput .= str_replace( array ( '&', '"', "'", '<', '>', "\r\n" , "\n" ), array ( '&amp;' , '&quot;', '&#39;' , '&lt;' , '&gt;', '<br>', '<br>' ), STR_MSG_TO . " " . $receiver_id );
    }
	}
  $strOutput .= ": ";
  $strOutput .= str_replace( array ( '&', '"', "'", '<', '>', "\r\n" , "\n" ), array ( '&amp;' , '&quot;', '&#39;' , '&lt;' , '&gt;', '<br>', '<br>' ), rtrim($message_text) );		
  
  //determina o layout de exibiçao, usando classes especiais
  // se o papel atual for professor ou se for uma pergunta 
  if (!empty($cor)) { $estilo='color:'.$cor.';'; }
  if ($strOutput[strlen($strOutput)-1]=='?') { $estilo.='font-weight:bold;'; }  
    
  if ($par) { $par=0; $classe='linhaPar';} else { $par=1; $classe='linhaImpar';}
  
  $output .= "<div class=\"msgNormal ".$classe."\" style=\"".$estilo."\" >".$strOutput."</div>";
}
...
$sql = "SELECT *,UNIX_TIMESTAMP(MSG.DATA) U_M_TIMESTAMP ".
...
while ($myrow = mysql_fetch_array($result))  {
  ...
  talk_print_message($myrow['COD_PESSOA'],$myrow['NOME_ENVIA'], $myrow['NOME_CHAT'],$myrow['NOME_RECEBE'], $myrow['MENSAGEM'], $myrow['U_M_TIMESTAMP'], $myrow['RESERVADO'],$myrow['COR']);
}
?>
<script>
/*  em caso de erro faz reload */
function goAhead() {
